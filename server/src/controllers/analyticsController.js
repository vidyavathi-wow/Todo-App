...
exports.getAnalytics = async (req, res) => {
  try {
    ...
    const statusPercentages = Object.fromEntries(
      Object.entries(statusCounts).map(([k, v]) => [k, ((v / totalTodos) * 100).toFixed(1)])
    );

    const priorityPercentages = Object.fromEntries(
      Object.entries(priorityCounts).map(([k, v]) => [k, ((v / totalTodos) * 100).toFixed(1)])
    );

    const categoryPercentages = Object.fromEntries(
      Object.entries(categoryCounts).map(([k, v]) => [k, ((v / totalTodos) * 100).toFixed(1)])
    );
